...
export default function EventShareButtons({ eventName, eventUrl, visibility, eventDate }) {
  ...
  const dateStr = eventDate ? moment(eventDate).format("MMM D, YYYY") : "";

  const shareText = `Join me at ${eventName}${dateStr ? ` on ${dateStr}` : ""} on FestChain 🎟️\nSecure digital ticket, QR check-in, and FestCoin pilot rewards.\n${eventUrl}`;
  ...
  const handleInstagram = async () => {
    ...
    navigator.clipboard.writeText(eventUrl).then(() => {
      toast({ title: "Event link copied for Instagram!" });
    }).catch(() => {});
  };
  ...
  const buttons = [
    ...
    { label: copied ? "Copied!" : "Copy Link", icon: copied ? Check : Copy, onClick: handleCopy },
  ];

  if (typeof navigator !== "undefined" && navigator.share) {
    buttons.push({ label: "Share", icon: Share2, onClick: handleNativeShare });
  }
    ...
    <div className="space-y-3">
      {isPrivate && (
        <div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 text-xs text-amber-400">
          ...
          <p>This event is private. Only people with the link or invited guests may access it.</p>
        </div>
      )}
