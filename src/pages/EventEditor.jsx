  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between">
        <h1 className="font-heading font-bold text-3xl text-white">{isEdit ? "Edit Event" : "Create Event"}</h1>
        <Button onClick={handleSave} disabled={saving} className="bg-primary hover:bg-primary/90 text-white font-semibold rounded-xl h-10 px-5">
          {saving ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Saving...</> : <><Save className="w-4 h-4 mr-2" />{isEdit ? "Update Event" : "Publish Event"}</>}
        </Button>
      </div>

      {/* BASICS */}
      <Section title="Event Basics" icon={Calendar}>
        <Field label="Event Name *"><Input value={form.title} onChange={e => set("title", e.target.value)} className="rounded-xl" placeholder="Summer Solstice" /></Field>
      ...
      {/* TICKET PHASES */}
      <Section title="Ticket Phases" icon={TicketIcon} subtitle="Default phases are preloaded. Only the active phase within its sales window is buyable; phases auto-advance.">
        ...
        <button onClick={addPhase} className="flex items-center gap-1.5 text-primary text-sm font-medium hover:underline mt-2"><Plus className="w-4 h-4" /> Add Phase</button>
      </Section>

      {/* LINEUP */}
      <Section title="Lineup / DJs" icon={Music}>
        <div className="space-y-3">
        ...
        <button onClick={addLineup} className="flex items-center gap-1.5 text-primary text-sm font-medium hover:underline mt-2"><Plus className="w-4 h-4" /> Add DJ / Artist</button>
      </Section>

      {/* SCHEDULE */}
      <Section title="Schedule" icon={Clock} subtitle="Doors open, DJ set, main act, closing — show the night's timeline.">
        ...
        <button onClick={addSchedule} className="flex items-center gap-1.5 text-primary text-sm font-medium hover:underline mt-2"><Plus className="w-4 h-4" /> Add Schedule Item</button>
      </Section>
